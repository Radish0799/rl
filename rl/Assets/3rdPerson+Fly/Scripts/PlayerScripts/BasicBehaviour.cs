...
public class BasicBehaviour : MonoBehaviour
{
	...
	private int[] mlDiscreteActions = new int[4]; // [forward, right, rotate, sprint]
	
	// 新增：標記是否為ML模式
	private bool isMLMode = false;
	...
	// 檢查是否為ML模式
	public bool IsMLMode => isMLMode;

	// ML Agent 設定離散動作輸入的方法
	public void SetMLDiscreteActions(int[] actions)
	{
		if (actions.Length >= 4)
		{
			...
			mlDiscreteActions[3] = actions[3]; // sprint
			useMLInput = true;
			isMLMode = true;
		}
	}
	...
	// 設定ML模式開關
	public void SetMLMode(bool enableML)
	{
		isMLMode = enableML;
		if (!enableML)
		{
			useMLInput = false;
			System.Array.Clear(mlDiscreteActions, 0, mlDiscreteActions.Length);
	...
	void Awake ()
	{
		...
		if (playerCamera != null)
		{
			...
			if (camScript == null)
			{
				Debug.LogWarning("ThirdPersonOrbitCamBasic script not found on camera");
			}
		}
		else
		{
			Debug.LogWarning("Player camera not assigned in BasicBehaviour");
		}
		...
		if (collider != null)
		{
			colExtents = collider.bounds.extents;
		}
		else
		{
			Debug.LogWarning("No Collider component found for ground detection");
			colExtents = Vector3.one; // 默認值
		}
	}

	void Update()
	{
		// 處理輸入：區分 ML Agent 和手動輸入
		if (useMLInput && isMLMode)
		{
			// 轉換離散動作為連續值
			...
			// sprint: 0=不衝刺, 1=衝刺
			sprint = (mlDiscreteActions[3] == 1);

			useMLInput = false; // 重置標記，等待下一幀的ML輸入
		}
		else if (!isMLMode)
		{
			// 使用手動輸入
	...
	{
		// 在ML模式下不要自動重新定位，避免與ML旋轉控制衝突
		if (isMLMode) return;
		
		if(lastDirection != Vector3.zero && rBody != null)
		{
			lastDirection.y = 0;
	...
	{
		// 改進的地面檢測，增加安全檢查
		if (colExtents == Vector3.zero)
		{
			return true; // 如果沒有collider信息，假設在地面上
		}
		
		Vector3 rayStart = transform.position + Vector3.up * (2 * colExtents.x);
		Ray ray = new Ray(rayStart, Vector3.down);
		float rayDistance = colExtents.x + 0.2f;
		
		return Physics.SphereCast(ray, colExtents.x, rayDistance, LayerMask.GetMask("Ground", "Default"));
	}
}
...
public abstract class GenericBehaviour : MonoBehaviour
{
	...
	void Awake()
	{
		...
		{
			Debug.LogError("BasicBehaviour component not found on " + gameObject.name);
		}
	...
	public virtual bool AllowSprint()
	{
		return canSprint;
	}
}